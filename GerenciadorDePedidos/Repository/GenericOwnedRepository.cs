...
namespace GerenciadorDePedidos.Repository
{
        ...
        where TDTO : class, IDTO, IOwnedDTO
    {
        ...
        public virtual async Task<TDTO> GetMineById(ClaimsPrincipal User, string id)
        {
            ...
            {
                TEntity? entity =
                    await context.Set<TEntity>().Where(e => e.UserId == userId && e.Id == id).FirstOrDefaultAsync();
        ...
        public virtual async Task<TDTO> UpdateMine(ClaimsPrincipal User, TDTO dto)
        {
            ...
            {
                TEntity? toUpdate =
                    await context.Set<TEntity>().Where(e => e.UserId == userId && e.Id == dto.Id).FirstOrDefaultAsync();
